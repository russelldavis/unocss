  ...
  do {
    ...
    content = content.replace(
      ...
      (from, pre: string, sep: string, body: string, groupOffset: number) => {
        ...
        // the next pass). The contents won't be used, because we'll end up getting the original
        // contents from itemsMap.
...
export function expandVariantGroup(str: string | MagicString, separators = ['-', ':'], depth = 5) {
  const {
    expanded,
  } = parseVariantGroup(str, separators, depth)

  return typeof str === 'string' ? expanded.toString() : expanded
}
